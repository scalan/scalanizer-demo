...
trait LinearAlgebraOps { self: LinearAlgebra =>
  ...
  object LA {
    ...
    def ddmvm(m: Array[Array[Double]], v: Array[Double]): Array[Double] = {
      ...
      val plusMonoid: NumMonoid[Double] = new PlusMonoid[Double](doubleNumer)

      val width = m(0).length
      val matrix: Matr[Double] = {
        new DenseMatr(Col((m: Array[Array[Double]]).map { r: Array[Double] => new DenseVec(Col(r)) }), width)
      }
      val vector: Vec[Double] = new DenseVec(Col(v))
