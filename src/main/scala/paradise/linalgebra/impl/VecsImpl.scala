  ...
  type _$26
      ...
      trait Vecs extends Base { self: LinearAlgebraDsl =>
        trait Vec[T] extends Reifiable[Vec[T]] {
          ...
          def dot(other: Rep[Vec[T]])(implicit n: Numer[T], m: NumMonoid[T]): Rep[T]
        };
        abstract class DenseVec[T](val items: Rep[Col[T]])(implicit val eeT: Elem[T]) extends Vec[T] with Product with Serializable {
          ...
          def dot(other: Rep[Vec[T]])(implicit n: Numer[T], m: NumMonoid[T]): Rep[T] = other.items.zip(items).map(fun(((v: Rep[scala.Tuple2[T, T]]) => n.times(v._1, v._2)))).reduce
        };
        trait VecCompanion;
