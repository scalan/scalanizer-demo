...
//            val matrix: Rep[Matrix[Double]] = CompoundMatrix(Collection(m.map(fun(((r: Rep[Array[Double]]) => DenseVector(Collection(r)))))), width);
//            val vector: Rep[Vector[Double]] = DenseVector(Collection(v));
