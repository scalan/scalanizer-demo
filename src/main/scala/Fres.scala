...
trait Fres {
  ...
  trait Free[F[_], A] {
    ...
    def flatMapBy[B](f: A => Free[F,B]): Free[F,B] = Bind(this, f)

    def mapBy[B](f: A => B): Free[F,B] = flatMap(a => Return(f(a)))
    def map[B](f: A => B): Free[F,B] = mapBy(f)

    def step: Free[F,A] = this
  }

  case class Return[F[_],A](val a: A) extends Free[F,A]
  {
    override def flatMap[B](f: A => Free[F,B]): Free[F,B] = f(a)
    override def flatMapBy[B](f: A => Free[F,B]): Free[F,B] = f(a)
  }

  case class Suspend[F[_],A](val a: F[A]) extends Free[F, A]

  case class Bind[F[_],S,B](val a: Free[F, S], val f: S => Free[F,B]) extends Free[F,B] {
