  ...
  type _$26
      ...
      trait Vecs extends Base { self: LinearAlgebraDsl =>
        ...
        abstract class DenseVec[T](val items: Rep[Col[T]])(implicit val ctT: Rep[ClassTag[T]], val eeT: Elem[T]) extends Vec[T] {
          ...
          def dot(other: Rep[Vec[T]])(implicit n: Rep[Num[T]], m: Rep[NumMonoid[T]]): Rep[T] = other.items.zip[T](DenseVec.this.items).map[T](fun(((v: Rep[scala.Tuple2[T, T]]) => n.times(v._1, v._2))))(DenseVec.this.ctT).reduce(m)
        };
        trait VecCompanion;
