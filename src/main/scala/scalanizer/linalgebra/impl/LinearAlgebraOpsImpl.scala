      ...
      trait LinearAlgebraOps extends Base { self: LinearAlgebraDsl =>
        ...
        trait LACompanion {
          def ddmvm(m: Rep[WArray[WArray[Double]]], v: Rep[WArray[Double]]): Rep[WArray[Double]] = {
            ...
            val matrix: Rep[DenseMatr[Double]] = {
              DenseMatr(
                LinearAlgebraOps.this.Col.apply[Vec[Double]](
                  WPredef.refArrayOps[WArray[Double]](m).map[DenseVec[Double], WArray[Vec[Double]]](
                    fun(
                      ((r: Rep[WArray[Double]]) =>
                        DenseVec(LinearAlgebraOps.this.Col.apply[Double](r))
                          (((self.DoubleElement): Elem[Double]))
                      )
                    )
                  )(
                      (WArray.canBuildFrom[DenseVec[Double]]).asRep[WCanBuildFrom[WArray[WArray[Double]],DenseVec[Double], WArray[Vec[Double]]]]
                    )
                ), width
              )(((self.DoubleElement): Elem[Double]))
            };
