  ...
  type _$13
      ...
      trait Cols extends Base { self: DemoDsl =>
        ...
        trait ColOverArrayCompanion {
          def safeApply(arr: Rep[MyArrWrapper[Int]], index: Rep[Int]): Rep[Int] = IF(arr.length.<=(index)).THEN(arr.apply(arr.length.-(toRep(1)))).ELSE(IF(index.<(toRep(0))).THEN(arr.apply(toRep(0))).ELSE(arr.apply(index)))
        }
      };
