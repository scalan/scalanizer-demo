package segms {
  ...
  ((diff): Rep[Int])
      ...
      abstract class Slice(val start: Rep[Int], val length: Rep[Int]) extends Segm {
        class StartBox(val value: Rep[Int]);
        def end: Rep[Int] = new StartBox(start).value.+(length);
