  ...
  type _$3
    ...
    trait Patterns extends Base { self: PatternsDsl =>
      trait Pattern extends Reifiable[Pattern] {
        def get(a: Rep[Any], b: Rep[Int]): Rep[String] = IF(a.isInstanceOf[Int]).THEN({
  val c = a;
  c.toString
}).ELSE(toRep("Unknown"))
      };
