...
})).THEN(toRep("Zero")).ELSE(IF(a.isInstanceOf[OnePat]).THEN({
  lazy val restPatterns = IF(a.isInstanceOf[Rep[Int]]).THEN({
  val c: Rep[Int] = ((a.asInstanceOf[Rep[Int]]): Rep[Int]);
  toRep("Int")
}).ELSE(IF(a.==(toRep(0))).THEN(toRep("0 or 1")).ELSE(IF(a.==(toRep(1))).THEN(toRep("0 or 1")).ELSE(IF(a.==(b)).THEN(toRep("b")).ELSE(IF(a.==(scala.math.Pi)).THEN(toRep("3.14")).ELSE(toRep("Unknown"))))));
  val deadbeef: OnePat = ((a.asInstanceOf[OnePat]): OnePat);
  val optionName = OnePat.unapply(deadbeef);
  IF(optionName.isEmpty.`unary_!`).THEN({
  val y = optionName.get;
  toRep("one").+(y.toString)
}).ELSE(restPatterns)
}).ELSE(IF(a.isInstanceOf[Rep[Int]]).THEN({
  val c: Rep[Int] = ((a.asInstanceOf[Rep[Int]]): Rep[Int]);
  toRep("Int")
}).ELSE(IF(a.==(toRep(0))).THEN(toRep("0 or 1")).ELSE(IF(a.==(toRep(1))).THEN(toRep("0 or 1")).ELSE(IF(a.==(b)).THEN(toRep("b")).ELSE(IF(a.==(scala.math.Pi)).THEN(toRep("3.14")).ELSE(toRep("Unknown"))))))))
      };
