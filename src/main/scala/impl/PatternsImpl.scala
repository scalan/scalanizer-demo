  ...
  type _$3
    ...
    trait Patterns extends Base { self: PatternsDsl =>
      trait Pattern extends Reifiable[Pattern] {
        def get(a: Rep[Any], b: Rep[Int]): Rep[String] = IF(a.==(toRep(0))).THEN(toRep("0")).ELSE(IF(a.==(b)).THEN(b.toString).ELSE(toRep("Unknown")))
      };
