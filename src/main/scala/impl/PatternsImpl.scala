  ...
  type _$3
    ...
    trait Patterns extends Base { self: PatternsDsl =>
      trait Pattern extends Reifiable[Pattern] {
        def get(a: Rep[AnyVal]): Rep[String] = IF(a.==(toRep(0))).THEN(toRep("0")).ELSE(IF(a.isInstanceOf[Rep[Int]]).THEN({
  val i = a;
  i.toString
}).ELSE(toRep("Unknown")))
      };
