  ...
  type _$3
    ...
    trait Patterns extends Base { self: PatternsDsl =>
      trait Pattern extends Reifiable[Pattern] {
        def get(a: Rep[Any], b: Rep[Int]): Rep[String] = IF(a.isInstanceOf[Rep[Int]]).THEN(toRep("Int")).ELSE(IF(a.==(toRep(0))).THEN(toRep("0 or 1")).ELSE(IF(a.==(toRep(1))).THEN(toRep("0 or 1")).ELSE(IF(a.==(b)).THEN(toRep("b")).ELSE(IF(a.==(scala.math.Pi)).THEN(toRep("3.14")).ELSE(toRep("Unknown"))))))
      };
